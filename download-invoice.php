...
try {
    ...
    if (!empty($serviceIds)) {
        ...
        $stmtServices = $db->prepare(
            "SELECT service_id, service_name,sac_code FROM services WHERE service_id IN ($placeholders)",
        );
    ...
    $pdf = new Fpdi();
    $templatePath = isset($invoiceSettings["template"])
        ? $invoiceSettings["template"]
        : "public/assets/invoice-temp/invoice-temp-1.pdf";
    ...
    $pdf->SetXY(20, 50);
    $pdf->Cell(22, 10, $companySettings["address"], 0, 0); // Render "Invoice No:" in black, normal font
    ...
    $width =
        max(
            $pdf->GetStringWidth($first_line),
            $pdf->GetStringWidth($second_line),
        ) + 5; // Use widest line for width
    ...
    foreach ($services as $item) {
        ...
        $pdf->Cell(
            0,
            $lineHeight,
            $currencySymbol . "." . $invoice["total_amount"],
            0,
            0,
        );
    ...
    $pdf->Output(
        "D",
        ucfirst(str_replace(" ", "-", $invoice["invoice_title"])) .
            "-" .
